...
- **Vollständiges Design-System & Schrift-Skalierung** (NFR-07): erster Pass erledigt in App 0.9.4 (siehe CHANGELOG). Eingezogene `_KpiCard`-Semantik plus drei weitere Bausteine: ~~kontrastsichere Theme-Tokens~~ (`buildAppTheme` in `lib/theme.dart`: 48×48-Tap-Targets via `IconButtonTheme`, 2-px-Focus-Ring + `filled` Input-Decoration für WCAG 1.4.11, rounded `CardTheme`, 16×12 Button-Padding), ~~MediaQuery-gestützte `textScaleFactor`-Anpassungen~~ (`MaterialApp.builder` in `main.dart` klemmt `MediaQuery.textScalerOf` auf max. 1.6× — höher zerlegt aktuell die mehrspaltigen Formular-Sections), ~~VoiceOver/TalkBack-Labels auf Formular-Feldern~~ (`NumberField` / `IntField` / `StringField` in `widgets/forms/_field.dart` wickeln das `TextFormField` jetzt in `Semantics(textField: true, label: …, hint: helpText)` + `ExcludeSemantics`; neue optionale `semanticsLabel`-Parameter erlauben Screen-Reader-only Overrides). **Weiter offen**: volle 200%-Skalierung (benötigt responsive Form-Layouts statt der heutigen festen Mehrspalter), Dynamic Colors (Material You via `dynamic_color`-Paket), durchgängiger WCAG-2.2-AA-Audit-Pass durch eine externe Auditfirma sowie sichtbare `InputDecoration.helperText`-Rendering-Variante (heute nur als Tooltip + Semantics-Hint, nicht permanent sichtbar). Nächste Triggerschwelle: erstes externes UX-Audit oder erste Kundenbeschwerde wegen 200%-Skalierung.
