...
void main() {
  group('SyntheticIrradianceSource', () {
    test('returns zero outside of the daylight window', () {
      ...
      final night = source.sampleFor(WeatherQuery(
        array: array, dayOfYear: 1, hourOfDay: 2, latitudeDeg: 50,
      ));
    ...
    test('summer noon irradiance exceeds winter noon at the same site', () {
      const source = SyntheticIrradianceSource();
      const array = PvArray(id: 'a', label: 'A', peakKw: 5, azimuthDeg: 180, tiltDeg: 35, inverterId: 'i');
      final summer = source.sampleFor(WeatherQuery(
        array: array, dayOfYear: 172, hourOfDay: 12.5, latitudeDeg: 50,
      ));
      final winter = source.sampleFor(WeatherQuery(
        array: array, dayOfYear: 355, hourOfDay: 12.5, latitudeDeg: 50,
      ));
    ...
    test('south-facing array beats east-facing at noon', () {
      ...
      final s = source.sampleFor(WeatherQuery(
        array: south, dayOfYear: 172, hourOfDay: 12.5, latitudeDeg: 50,
      ));
      final e = source.sampleFor(WeatherQuery(
        array: east, dayOfYear: 172, hourOfDay: 12.5, latitudeDeg: 50,
      ));
    ...
    test('default ambient is 25 °C — temperature derating defaults to zero impact', () {
      const source = SyntheticIrradianceSource();
      const array = PvArray(id: 'a', label: 'A', peakKw: 5, azimuthDeg: 180, tiltDeg: 35, inverterId: 'i');
      final sample = source.sampleFor(WeatherQuery(
        array: array, dayOfYear: 172, hourOfDay: 12.5, latitudeDeg: 50,
      ));
      ...
      for (final day in const [1, 60, 120, 172, 240, 300, 365]) {
        for (var h = 0.5; h < 24; h += 1.0) {
          final f = SyntheticIrradianceSource.normalizedPowerFactor(
            array: array, dayOfYear: day, hourOfDay: h, latitudeDeg: 50,
          );
  ...
  group('legacy engine behaviour preserved', () {
    test('with default array (tempCoeff=0) annual yield stays positive and pvAc < pvDc', () {
      ...
      // must not perturb yields when temperature derating is off. Exact
      // numerical equivalence is asserted in the test below by replaying
      // the formula directly; here we just confirm the simulator still
      // produces sensible monotone results.
